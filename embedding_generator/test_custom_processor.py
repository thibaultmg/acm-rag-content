...
import tempfile
import unittest
from custom_processor import slugify, CustomMetadataProcessor, get_anchor_id, AnchorLinkNodeParser
...
class TestCustomProcessor(unittest.TestCase):
    ...
    def test_url_function_main_to_index(self):
        ...
        cases = [
            ...
            (os.path.join(root, "docs", "acm", "folder", "main.md"), "http://example.com/folder/index"),
            (os.path.join(root, "docs", "acm", "main_file.md"), "http://example.com/main_file"),
            (os.path.join(root, "docs", "acm", "main"), "http://example.com/index"),
            (os.path.join(root, "docs", "acm", "domain.md"), "http://example.com/domain"),
        ]
    ...
    def test_url_function_rename_map(self):
        root = os.path.abspath(os.sep)
        cases = [
            (os.path.join(root, "docs", "combined", "acm_2.15", "main.md"), "http://example.com/acm/2.15/index"),
            (os.path.join(root, "docs", "combined", "thanos_latest", "query.md"), "http://example.com/thanos/latest/query"),
            (os.path.join(root, "docs", "combined", "unmapped", "file.md"), "http://example.com/unmapped/file"),
        ]
        
        rename_map = {
            "acm_2.15": "acm/2.15",
            "thanos_latest": "thanos/latest"
        }
