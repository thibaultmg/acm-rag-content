...
def main() -> int:
    ...
    parser.add_argument("-p", "--product", type=str, default="acm", help="Product name (e.g., acm, thanos). Default: acm")
    parser.add_argument("-v", "--version", type=str, default="2.15", help="Documentation version (e.g., 2.15, latest). Default: 2.15")
    ...
    if not OUTPUT_DIR.is_relative_to(base_output_dir):
        print(f"Error: Invalid product or version path (path traversal detected): {OUTPUT_DIR}", file=sys.stderr)
        return 1
