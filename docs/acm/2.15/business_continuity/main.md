...
- See AWS S3 compatible backup storage providers in the OpenShift
  Container Platform documentation for a list of supported Velero
  storage providers.
...
3.  On the restore hub cluster, use the upgrade operation to upgrade to
    the Red Hat Advanced Cluster Management operator version that you
    want to use.
...
### Backup and restore for hosted control plane overview

When using Red Hat Advanced Cluster Management to manage hosted clusters
with multicluster engine operator hosting clusters, you can back up and
restore the management capability of both the Red Hat Advanced Cluster
Management hub cluster and the hosted clusters.
...
When using Red Hat Advanced Cluster Management to manage hosted clusters
with multicluster engine operator hosting clusters, you need to know the
scope of the Red Hat Advanced Cluster Management backup and recovery
operation.

Red Hat Advanced Cluster Management only recovers the management
capability of multicluster engine operator hosting clusters and the
attached hosted clusters. This includes the following functional areas:

- Discovery and import configuration: Auto-discovery policies and
  `DiscoveredCluster` resources

- Management resources: Policies, applications, and governance
  configurations applied to hosted clusters

- Backup labels: Resources marked with
  `cluster.open-cluster-management.io/backup` labels

- Add-on configurations: HyperShift add-on settings and deployment
  configurations

The following functional areas are not within the disaster recovery
scope:

- multicluster engine operator hosting clusters: Physical infrastructure
  and cluster lifecycle

- Hosted clusters: The actual hosted control planes and their workloads

- Hosted cluster data: Applications, persistent volumes, and user data
  within hosted clusters

For complete disaster recovery in hosted cluster environments, implement
a multi-layered backup strategy that includes the following processes:

- Red Hat Advanced Cluster Management hub cluster backup

  - Purpose: Restore management capabilities and discovery
    configurations

  - Scope Red Hat Advanced Cluster Management resources, policies,
    applications, and addon configurations

  - Implementation: Use Red Hat Advanced Cluster Management’s
    cluster-backup-operator
...
- Hosted cluster backup
  ...
  - Implementation: Follow OpenShift Hosted Control Plane Disaster
    Recovery documentation.

- The multicluster engine operator hosting cluster backup

  - Purpose: Backup the infrastructure hosting the control planes

  - Scope: multicluster engine operator cluster state and configurations

  - Implementation: Standard OpenShift Container Platform backup
    procedures

Continue to the Restoring hosted control plane management configuration
for detailed steps on restoring hosted clusters managed by Red Hat
Advanced Cluster Management.

### Restoring hosted control plane management configuration

When restoring hosted clusters that are managed by Red Hat Advanced
Cluster Management, follow steps to ensure a successful recovery of both
the Red Hat Advanced Cluster Management hub cluster and the hosted
clusters.

Follow OpenShift Hosted Control Plane Disaster Recovery documentation.
...
See the following guidance to restore hosted control plane management
configuration

</div>

1.  Restore the Red Hat Advanced Cluster Management hub cluster. See
    Restoring a backup.

2.  Deploy new Red Hat Advanced Cluster Management hub with same
    configuration as the original. See Installing the backup and restore
    operator

3.  Enable backup component and configure OADP.

4.  Restore Red Hat Advanced Cluster Management backup by using the
    `cluster-backup-operator`.

5.  Verify discovery and auto-import policies are restored. See
    Discovering multicluster engine operator hosted clusters in Red Hat
    Advanced Cluster Management

6.  Verify that multicluster engine operator hosted environment is
    reconnected to the restored Red Hat Advanced Cluster Management hub
    cluster. See Restoring a backup. The following clusters and
    resources are restored when the backup and restore is successful:

    - multicluster engine operator hosting clusters automatically
      reconnect to restored Red Hat Advanced Cluster Management hub
      cluster.
...
7.  Recover your hosted cluster. Hosted clusters require a separate
    recovery by using individual backup procedures. The Red Hat Advanced
    Cluster Management hub cluster automatically rediscovers clusters
    after the APIs become available. See
    https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/hosted_control_planes/high-availability-for-hosted-control-planes#hcp-disaster-recovery-oadp-auto.

For other guidance on backup and restore, see the Backup and restore
documentation.

For more information about hosted control plane, see About hosted
control planes with multicluster engine operator.
...
- Ensure that you can run a restore operation for backup types by
  completing the Using the restore operation for backup types process.
